...
if(isset($_POST["find"]))
{
    ...
    $search_Result = mysqli_query($link,"SELECT * FROM actor WHERE actor_id = $data[0]");
...
<div class="col-lg-4">
  <h2>Actor Form</h2>
  <form action="" name="Actor Table" method="post">
...
{
    $data = getPosts();
    $insert_Query = "INSERT INTO `actor`(`actor_id`,`first_name`,`last_name`, `last_update`) VALUES ('$data[0]','$data[1]','$data[2]','$data[3]')";
    try{
...
{
    $data = getPosts();
    $delete_Query = "DELETE FROM `actor` WHERE `actor_id` = $data[0]";
    try{
...
{
    $data = getPosts();
    mysqli_query($link, "UPDATE `actor` SET `first_name`='$data[1]',`last_name`='$data[2]',`last_update`='$data[3]', WHERE `actor_id` = $data[0]");
    if(mysqli_affected_rows($link) > 0)
    {
        echo 'Data Updated';
    }else{
